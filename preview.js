// --- Variables ---
    ...
    function showImage(index) {
        // Ensure elements were found in the DOM at script load time
        // This check is primarily redundant AFTER initial DOMContentLoaded checks
        // but kept for safety if called in unexpected ways.
        // The main error should be caught in the DOMContentLoaded block.
         if (!previewImage || !imageTitleElement || !imageDescElement || !downloadBtnElement || !bgElement || !prevImageElement || !nextImageElement) {
             console.error("showImage called but one or more required elements are null.");
             // Avoid further errors if elements are missing
             return;
         }
        ...
        if (images.length === 0) {
             ...
             // Handle UI for no images case if needed
             previewImage.src = ""; // Clear image if any
             imageTitleElement.innerText = "No Images";
             imageDescElement.innerText = "";
             downloadBtnElement.href = "#";
             bgElement.src = "";
             prevImageElement.style.display = 'none';
        ...
        if (currentIndex === newIndex && previewImage && previewImage.src === nextImage.src) {
            // console.log("Attempted to show the same image, stopping.");
            // Still ensure previews are correct in case something was off (though showImage should handle this)
             if (images.length > 1) {
                 const prevIndex = (currentIndex - 1 + images.length) % images.length;
                const nextIndex = (currentIndex + 1 + images.length) % images.length; // Ensured wrap-around
        ...
        // Set opacity to 0 BEFORE changing src if you want a fade effect (CSS transition)
        // This part needs the animation logic added as discussed previously if you want fade.
        // For now, this is the direct change:
        ...
        if (images.length > 1) { // Only show previews if there's more than one image
            ...
            const prevIndex = (currentIndex - 1 + images.length) % images.length;
            const nextIndex = (currentIndex + 1 + images.length) % images.length; // Ensure wrap-around here too

            // Set the source of the preview image elements
            if (prevImageElement) {
                prevImageElement.src = images[prevIndex].src;
                prevImageElement.style.display = ''; // Ensure it's visible (assuming default is hidden)
            }
            if (nextImageElement) {
    ...
    // Defined outside DOMContentLoaded so it can be accessed by event listeners set up there.
    function saveImage() {
...
        // Or use the direct image URL if you prefer: const urlToShare = previewImage ? previewImageImage.src : null; // Corrected typo here if using direct src
...
        let startingIndex = images.findIndex(img => img.src === decodedImageUrl);
        ...
        // This check is essential to prevent errors later if elements are missing.
        if (!previewImage || !imageTitleElement || !imageDescElement || !downloadBtnElement || !bgElement || !prevImageElement || !nextImageElement) {
             console.error("One or more required preview page elements not found in the DOM. Check your HTML IDs.");
             document.body.innerHTML = "<h2>Error loading page! Some required elements are missing from the HTML.</h2>";
...
            }
        ...
        if (nextImageElement) {
            nextImageElement.addEventListener("click", function() {
                ...
                const nextIndex = (currentIndex + 1 + images.length) % images.length; // Corrected wrap-around here
